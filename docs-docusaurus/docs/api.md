...
- **`auto_sync_medmarket_enabled`** / **`auto_sync_medmarket_time`** / **`auto_sync_medmarket_weekdays`** — обновление каталога **Медмаркет** из `ms_export` в `dg_medmarket_map` (код, наименование, тип; **`medmarket_code` не затирается**). МСК, `HH:MM`, по умолчанию `09:00`; дни недели — CSV **1=пн … 7=вс** (по умолчанию **`1,4`** — пн и чт). `task_type='medmarket'` в `auto_sync_runs`; журнал на `/processes.html`. Кнопка «Обновить каталог из МС» на `/medmarket.html` и «Запустить сейчас» в `/settings.html` — `POST /api/settings/auto-sync-run` с `task: "medmarket"` или `POST /api/medmarket/sync`.
