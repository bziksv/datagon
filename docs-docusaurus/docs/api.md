...
Полная карточка товара или комплекта из JSON API МойСклад (включая все атрибуты), для экрана «МойСклад» по клику на наименование. Полные JSON-карточки сохраняются в таблицу `ms_entity_details`: вечерняя синхронизация обновляет их массово, а одиночный клик сначала читает БД и только при отсутствии записи запрашивает API МойСклад, после чего сохраняет результат. Справочник имён атрибутов `metadata/attributes` кэшируется на **~60 минут** (см. `MS_ATTRS_META_CACHE_TTL_MS` в `routes/moysklad.js`).
