...
В каждой строке также **`presence`** (колонка **«Связь»** после ID): `my_products` (SKU в `my_products` после импорта CMS «Мои товары»), `ms` (код/артикул в `ms_export`), `ozon`/`wb`/`ym` + URL из `marketplace_export_rows`. Серые = нет связи, зелёные = есть. Не пишет placement в `dg_new_products` — только отображение после импортов. Кросс-вкладка Альмамед↔Маркеты — ссылка под ID (`← Альм. #N` / `→ Марк. #N`).
