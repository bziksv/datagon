...
**Профиль отсутствия** (`lib/datagonSupplierAbsenceProfile.js`): по `dg_product_zero_stock_log` строятся эпизоды нуля на каждый `code` → на SKU `recommended_replenishment_days` = **max(макс. простой, ceil(ср. эпизод), max(глобаль, оверрайд поставщика))** (короткий нуль не даёт «рек. 1»). **В формулу закупок** эти дни входят **на уровне товара**, если включено `sales_formula_sku_replenishment_enabled` и рек. выше базы: цепочка **SKU-рек. → supplier override → global** (`k = дни÷W`, `resolveEffectiveReplenishment`). Отдельно в формуле всегда есть **упущенные продажи** за окно A (`absenceDistinct × avgW`) — это рычаг **спроса**, а рек. дни — рычаг **горизонта**; вместе усиливают запас против нуля (не дубль одной штуки). Fingerprint кэша: `|rd:g` / `|rd:s:N` / `|rd:sku:N`. На `/supplier-analysis.html` и `/suppliers.html` **нет** «рек. дней на всего поставщика» / кнопки «Применить» — диагностика только эпизодов (долгие / частые короткие). Согласие с предложением — через **предлагаемый нес.остаток** на `/purchase.html`. Пороги бейджей: долгий ≥14 дн., частый короткий ≥3 эпизода со ср. длиной ≤3.
