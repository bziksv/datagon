...
- **`DATAGON_AUTO_SYNC_SCHEDULER=off`** — режим «только по кнопке» для **локальной разработки** с общей (продовой) БД: планировщик автосинхронизаций не стартует, локальный процесс не помечает чужие `running`-записи `auto_sync_runs` как «прервано»; кнопки **«Запустить сейчас»** (`POST /api/settings/auto-sync-run`) работают. Локально: **`npm run dev:local`**. На проде переменную **не задавать** — по умолчанию scheduler включён.
