...
| `sales_formula_*` | Формула продаж v2 на [карточке товара](/docs/product): W/A; **пополнение в днях** (`sales_formula_replenishment_days`, k=дни÷W); **`sales_formula_sku_replenishment_enabled`** (1/0) — авто-рек. дни по SKU (горизонт `k`); **`sales_formula_absence_analysis_days`** — окно для **упущенных шт в спросе**. Это **два разных рычага** против нуля на складе (не дубль одной поправки); вместе дают более жёсткий предлагаемый нес.остаток. «Базовый запас» / «для дорогих» — минимумы после кратности; «для редких» — ранняя ветка. См. `lib/datagonSalesFormula.js`. |
