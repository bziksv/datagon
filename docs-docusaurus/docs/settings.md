...
| `sales_formula_*` | Формула продаж v2 на [карточке товара](/docs/product): W/A, k; «Базовый запас» и «Базовый для дорогих…» **не** прибавляются к «сумма × k», но после кратности и макс. скачка задают **минимум** (обычный / дорогой); «Базовый для редких» — ветка редкого товара и дробь &lt; 1 шт. Порог дорогого, % упаковки, макс. скачок. См. `lib/datagonSalesFormula.js`. |
