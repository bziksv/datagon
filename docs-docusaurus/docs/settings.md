...
| `auto_sync_myproducts_*` / `auto_sync_marketplaces_*` / `auto_sync_huckster_*` / `auto_sync_db_size_*` / `auto_sync_dimensions_*` | Расписание фоновых задач по московскому времени (если включено). Галка «Размер БД и диска» (`auto_sync_db_size_*`) единовременно пересчитывает **обе** метрики дашборда: размер базы (`/api/processes/db-size`, виджет «Размер базы данных») и разбивку диска по папкам проекта (`/api/processes/disk-usage`, карточка «Дисковое пространство»). Также сюда входит **выгрузка пользовательских габаритов** в МойСклад (по умолчанию 21:00, серверный аналог кнопки «↗ В МС: все правки» на `/exports-dimensions.html`; журнал — `auto_sync_runs` task `dimensions` + `ms_dimensions_log` action `sync_ms`; прогресс — на `/processes.html` раздел «Габариты МС»). |
...
| `sales_formula_*` | Формула продаж v2 на [карточке товара](/docs/product): W/A, k; «Базовый запас» и «Базовый для дорогих…» **не** прибавляются к произведению «сумма с учётом отсутствий × k»; «Базовый запас для товаров с редкими продажами» — ветка редкого товара и минимум при дроби &lt; 1 шт после кратности/макс. скачка (кратность &lt; 1 и т.п.). Порог дорогого, кратность, % упаковки, макс. скачок. См. `lib/datagonSalesFormula.js`. |

Ручной запуск задач из той же карточки (кнопки «Запустить сейчас» → `POST /api/settings/auto-sync-run`): ответ сервера показывается **цветной плашкой** вверху блока (принято в очередь / уже выполняется или уже в очереди / ошибка), с указанием **занятости воркера** и **текущей очереди**; во время ожидания ответа кнопка блокируется. Подробный ход — на странице [«Логи фоновых процессов»](/docs/processes).
