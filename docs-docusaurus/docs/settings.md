...
| `auto_sync_mssales_enabled` / `auto_sync_mssales_time` / `auto_sync_mssales_days` | Авто-импорт **Продаж МС** (`/ms-sales.html`). Дёргает `entity/demand` из МС API за окно `auto_sync_mssales_days` (1..1825, default **90**). Время — МСК (default **07:30**). Внутренне идёт через `routes/msSales.js → triggerSync()` (тот же путь, что POST `/api/ms-sales/sync`); прогресс и статус — на `/processes.html` (раздел «Продажи МС»), запись о прогоне — `auto_sync_runs` (`task_type='mssales'`, `message` с метриками: помечено удалёнными / воскрешено). Кнопка «Запустить сейчас» в карточке `/settings.html` ставит задачу в очередь автосинхронизации (`POST /api/settings/auto-sync-run` с `task: 'mssales'`). |
| `auto_sync_myproducts_*` / `auto_sync_moysklad_*` / `auto_sync_marketplaces_*` / `auto_sync_huckster_*` / `auto_sync_db_size_*` / `auto_sync_dimensions_*` | Расписание фоновых задач по московскому времени (если включено). Галка «Размер БД и диска» (`auto_sync_db_size_*`) единовременно пересчитывает **обе** метрики дашборда: размер базы (`/api/processes/db-size`, виджет «Размер базы данных») и разбивку диска по папкам проекта (`/api/processes/disk-usage`, карточка «Дисковое пространство»). Также сюда входит **выгрузка пользовательских габаритов** в МойСклад (по умолчанию 21:00, серверный аналог кнопки «↗ В МС: все правки» на `/exports-dimensions.html`; журнал — `auto_sync_runs` task `dimensions` + `ms_dimensions_log` action `sync_ms`; прогресс — на `/processes.html` раздел «Габариты МС»). |
