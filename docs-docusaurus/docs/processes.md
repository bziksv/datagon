...
- **МойСклад** — фоновый `POST /api/ms/sync`: статус смотрите здесь и через [`GET /api/ms/status`](/docs/api/#moysklad). Под блоком — **архив шагов в БД** за выбранный день: `dg_ms_sync_log` за этот день (поле `moyskladPersistedLogs` в [`GET /api/processes/overview`](/docs/api/)). «Текущая сессия (память процесса, до 30 строк)» отдаётся только для «сегодня» — память процесса не различает дат.
- **Автосинхронизация по расписанию** — разнесена по разделам с человеко-читаемыми названиями: **Мои товары**, **МойСклад**, **Маркетплейсы**, **Калькуляция Huckster**, **Размер БД**. У каждого раздела видны статус расписания (вкл/выкл и время старта по МСК) и **запуски этого раздела за выбранный день** (`auto_sync_runs.task_type`), с продолжительностью и значком `по расписанию`/`вручную`. Если за день не было — раздел показывает заглушку «За выбранный день запусков не было».
...
## История автозапусков (`auto_sync_runs`)

Строки вроде **`moysklad — running | старт: … | финиш: -`** означают: запись в БД создана при старте задачи из расписания/ручного «Запустить сейчас», а **`finished_at` ещё не проставлен**. Обычно после завершения синка появится финиш и статус `completed` / `failed`.

Если **финиш так и не появился** после рестарта Node (деплой, pm2, падение процесса), старая строка могла остаться «висеть»: сервер не вызывает `finish` для уже мёртвого процесса. После обновления Datagon при **следующем старте** такие `running` без даты финиша **закрываются** со статусом `interrupted` и пояснением в `message`; записи старше **24 часов** в `running` дополнительно закрываются фоном как `failed`. Имеет смысл сверить реальное состояние синка на **МойСклад** и в логах Node.
