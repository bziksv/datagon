...
**Срок хранения завершённых строк** (`finished_at` не пустой) задаётся в [Настройках](/docs/settings/) — карточка **«Журнал запусков автосинхронизации»** (`auto_sync_runs_retention_days`, по умолчанию **180 дней**). Автоочистка выполняется при старте сервера и каждые 12 часов; активные `running` без финиша не удаляются.
