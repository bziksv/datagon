 ...
 * 6) Иначе черновик до кратности: adjusted × (дни пополнения ÷ W) = adjusted × k.
 *    UI задаёт **дни** (`sales_formula_replenishment_days`); k = days/W (legacy-ключ coef синхронизируется при сохранении).
 *    «Базовый запас» и «Базовый для дорогих…» **не прибавляются** к произведению, но после кратности/макс. скачка итог **не ниже** соответствующего минимума по классификации с этапов «дорогой» / «редкий»: обычный → sales_formula_base_qty; дорогой → sales_formula_expensive_rare_min_qty (редкая ветка — sales_formula_rare_base_qty).
...
/**
 * Горизонт пополнения в днях → множитель k = days / W.
 * Источник истины для UI: sales_formula_replenishment_days.
 * Legacy: если дней нет — days ≈ round(sales_formula_replenishment_coef × W).
 * @param {object} a app_settings
 * @param {number} salesWindowDays
 * @param {number|null|undefined} [overrideDays] — явный оверрайд дней
 * @param {'supplier'|'sku'|undefined} [sourceHint] — метка источника при оверрайде
 */
function resolveReplenishmentFromSettings(a, salesWindowDays, overrideDays, sourceHint) {
    const W = Math.max(1, salesWindowDays);
    if (overrideDays !== undefined && overrideDays !== null && String(overrideDays).trim() !== '') {
        const d = Math.round(num(overrideDays, NaN));
        if (Number.isFinite(d)) {
            const days = Math.max(0, Math.min(3650, d));
            const src = sourceHint === 'sku' || sourceHint === 'supplier' ? sourceHint : 'supplier';
            return {
                replenishmentDays: days,
                replenishmentCoef: days / W,
                replenishmentSource: src,
            };
        }
    }
    const rawDays = a && a.sales_formula_replenishment_days;
    let days = null;
    if (rawDays !== undefined && rawDays !== null && String(rawDays).trim() !== '') {
        const d = Math.round(num(rawDays, NaN));
        if (Number.isFinite(d)) days = Math.max(0, Math.min(3650, d));
    }
    if (days == null) {
        const coefLegacy = num(a && a.sales_formula_replenishment_coef, 1 / 3);
        days = Math.max(0, Math.min(3650, Math.round(coefLegacy * W)));
    }
    return {
        replenishmentDays: days,
        replenishmentCoef: days / W,
        replenishmentSource: 'global',
    };
}

/**
 * Включён ли авто-подъём дней пополнения по SKU из эпизодов нуля.
 * По умолчанию включено (ключ отсутствует / пусто → true).
 */
function isSkuReplenishmentEnabled(appSettings) {
    const v = appSettings && appSettings.sales_formula_sku_replenishment_enabled;
...
/**
 * Эффективные дни для формулы по SKU:
 * 1) рек. дни из отсутствия, если режим включён и они выше базы (supplier|global);
 * 2) иначе оверрайд поставщика;
 * 3) иначе глобаль.
 * @param {object} appSettings
 * @param {{ supplierDays?: number|null, skuRecommendDays?: number|null }} [opts]
 */
function resolveEffectiveReplenishment(appSettings, opts) {
    const o = opts || {};
    const supplierDays =
        o.supplierDays !== undefined && o.supplierDays !== null && String(o.supplierDays).trim() !== ''
            ? o.supplierDays
            : null;
    const base = parseFormulaSettings(appSettings, {
        replenishmentDaysOverride: supplierDays,
    });
    let skuRec = null;
    if (o.skuRecommendDays !== undefined && o.skuRecommendDays !== null && String(o.skuRecommendDays).trim() !== '') {
        const r = Math.round(num(o.skuRecommendDays, NaN));
        if (Number.isFinite(r)) skuRec = Math.max(0, Math.min(3650, r));
    }
    if (isSkuReplenishmentEnabled(appSettings) && skuRec != null && skuRec > base.replenishmentDays) {
        const elevated = parseFormulaSettings(appSettings, {
            replenishmentDaysOverride: skuRec,
            replenishmentSourceHint: 'sku',
        });
        return {
            ...elevated,
            replenishmentSource: 'sku',
            skuRecommendDays: skuRec,
            supplierReplenishmentDays: supplierDays,
            skuReplenishmentEnabled: true,
        };
    }
    return {
        ...base,
        skuRecommendDays: skuRec,
        supplierReplenishmentDays: supplierDays,
        skuReplenishmentEnabled: isSkuReplenishmentEnabled(appSettings),
    };
}

function parseFormulaSettings(appSettings, opts) {
    const a = appSettings || {};
    const o = opts || {};
    const salesWindowDays = Math.max(7, Math.min(365 * 2, Math.round(num(a.sales_formula_sales_window_days, 90))));
    const rep = resolveReplenishmentFromSettings(
        a,
        salesWindowDays,
        o.replenishmentDaysOverride,
        o.replenishmentSourceHint,
    );
    return {
        replenishmentDays: rep.replenishmentDays,
        replenishmentCoef: rep.replenishmentCoef,
        replenishmentSource: rep.replenishmentSource || 'global',
        salesWindowDays,
...
/** Карта TRIM(supplier_key) → дни пополнения (только заданные в dg_supplier_settings). */
async function loadSupplierReplenishmentDaysMap(db) {
    const out = new Map();
    if (!db) return out;
    try {
        const [rows] = await db.query(
            `SELECT supplier_key, replenishment_days
               FROM dg_supplier_settings
              WHERE replenishment_days IS NOT NULL`,
        );
        for (const r of rows || []) {
            const key = String(r.supplier_key || '').trim();
            const d = Math.round(Number(r.replenishment_days));
            if (!key || !Number.isFinite(d)) continue;
            out.set(key, Math.max(0, Math.min(3650, d)));
        }
    } catch (e) {
        /* колонка ещё не создана — игнор */
    }
    return out;
}

async function loadSupplierReplenishmentDaysForKey(db, supplierKey) {
    const key = String(supplierKey || '').trim();
    if (!db || !key) return null;
    try {
        const [rows] = await db.query(
            `SELECT replenishment_days FROM dg_supplier_settings
              WHERE supplier_key = ? AND replenishment_days IS NOT NULL LIMIT 1`,
            [key],
        );
        if (!rows || !rows.length) return null;
        const d = Math.round(Number(rows[0].replenishment_days));
...
function computeSalesFormula(opts) {
    ...
    const LBL_K =
        s.replenishmentSource === 'sku'
            ? `Пополнение, дней (рек. по SKU из отсутствий, сейчас ${s.replenishmentDays} дн.; k = дни÷W = ${fmtStoryNum(s.replenishmentCoef)})`
            : s.replenishmentSource === 'supplier'
              ? `Пополнение, дней (оверрайд поставщика dg_supplier_settings.replenishment_days, сейчас ${s.replenishmentDays} дн.; k = дни÷W = ${fmtStoryNum(s.replenishmentCoef)})`
              : `Пополнение, дней (настройка sales_formula_replenishment_days, сейчас ${s.replenishmentDays} дн.; k = дни÷W = ${fmtStoryNum(s.replenishmentCoef)})`;
    ...
    if (isRare) {
        ...
        return {
            ...
            inputs: {
                ...
                replenishment_days: s.replenishmentDays,
                replenishment_coef: s.replenishmentCoef,
                replenishment_source: s.replenishmentSource || 'global',
                draft_core: null,
                base_qty: s.baseQty,
    ...
    pushEq(
        ...
        'Сумма с учётом отсутствий × (дни пополнения ÷ «Продажи за период»)',
        `${fmtIntSpace(Math.round(adjustedSalesSum + 1e-9))} × (${s.replenishmentDays} ÷ ${W}) = ${fmtIntSpace(Math.round(adjustedSalesSum + 1e-9))} × ${fmtStoryNum(s.replenishmentCoef)} = ${fmtStoryNum(draftCore)}`,
        `«${LBL_K}». Поля «Базовый запас» (sales_formula_base_qty) и «${LBL_EXP_RARE_MIN}» к этому произведению не добавляются.`,
    );
...
function buildFormulaContextLines(ctx) {
    ...
    return [
        { label: `Пополнение, дней (sales_formula_replenishment_days${s.replenishmentSource === 'supplier' ? ', оверрайд поставщика' : ''})`, value: `${s.replenishmentDays} дн.` },
        {
            label: `Множитель k = дни÷W (sales_formula_replenishment_coef)`,
            value: String(fmtStoryNum(s.replenishmentCoef)),
        },
        {
            label: 'Источник горизонта пополнения',
            value: s.replenishmentSource === 'supplier' ? 'поставщик (dg_supplier_settings.replenishment_days)' : 'глобальные настройки',
        },
        {
...
module.exports = {
    ...
    resolveReplenishmentFromSettings,
    resolveEffectiveReplenishment,
    isSkuReplenishmentEnabled,
    loadSupplierReplenishmentDaysMap,
    loadSupplierReplenishmentDaysForKey,
    pickMarketPriceRub,
    computeSalesFormula,
