...
function purchaseOrderPositionFromExportRow(r) {
    const pos = {
        quantity: Number(r.need_qty),
        price: priceToMinorUnits(r.buy_price),
        assortment: assortmentMeta(r.uuid, r.ms_entity_type),
        _code: r.code,
    };
    if (r.vat_rate != null && Number.isFinite(Number(r.vat_rate))) {
        pos.vat = Number(r.vat_rate);
    }
    if (r.vat_enabled != null) {
        pos.vat_enabled = !!r.vat_enabled;
    }
    return pos;
}
...
async function createPurchaseOrderForSupplier(db, appSettings, opts) {
    ...
    try {
        ...
        for (const r of withQty) {
            ...
            positions.push(purchaseOrderPositionFromExportRow({ ...r, uuid }));
        }
        ...
        const body = {
            ...
            positions: positions.map((p) => {
                ...
                if (p.vat != null && Number.isFinite(Number(p.vat))) out.vat = Number(p.vat);
                if (p.vat_enabled != null) out.vat_enabled = !!p.vat_enabled;
                return out;
            }),
        };
