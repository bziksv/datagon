...
const PAGE_DEFS = [
    ...
    { key: 'purchase', title: 'Закупки', htmlFile: 'purchase.html', navSlug: 'purchase', sortOrder: 45 },
...
function resolveRedirectAfterLogin(actor, thenRaw) {
    ...
    if (actor.username === 'admin') {
        if (
            norm &&
            norm.path &&
            norm.leaf !== 'login.html' &&
            norm.leaf !== 'no-access.html'
        ) {
            return norm.path;
        }
        return '/dashboard.html';
    }
    if (norm && norm.leaf !== 'login.html' && norm.leaf !== 'no-access.html' && !isHtmlLeafAccessHiddenForActor(actor, norm.leaf)) return norm.path;
