...
function buildFormulaFingerprint(appSettings) {
    ...
    return [
        ...
        a.sales_formula_sku_replenishment_enabled == null || a.sales_formula_sku_replenishment_enabled === ''
            ? '1'
            : String(a.sales_formula_sku_replenishment_enabled),
...
/**
 * Суффикс fp дней пополнения:
 * - `rd:g` — глобаль;
 * - `rd:s:N` — оверрайд поставщика;
 * - `rd:sku:N` — рек. дни SKU из отсутствия (выше базы);
 * - legacy `rd:N` — старый оверрайд поставщика (чтение через LIKE).
 * @param {number|null|undefined} daysOrOverride
 * @param {'global'|'supplier'|'sku'|object} [sourceOrOpts]
 */
function formulaReplenishmentFpSuffix(daysOrOverride, sourceOrOpts) {
    let source = 'global';
    let days = daysOrOverride;
    if (sourceOrOpts && typeof sourceOrOpts === 'object' && !Array.isArray(sourceOrOpts)) {
        source = sourceOrOpts.source || sourceOrOpts.replenishmentSource || 'global';
        if (sourceOrOpts.days != null) days = sourceOrOpts.days;
        else if (sourceOrOpts.replenishmentDays != null) days = sourceOrOpts.replenishmentDays;
    } else if (typeof sourceOrOpts === 'string') {
        source = sourceOrOpts;
    }
    if (source === 'sku') {
        const d = Math.round(Number(days));
        if (!Number.isFinite(d)) return 'rd:g';
        return 'rd:sku:' + Math.max(0, Math.min(3650, d));
    }
    if (source === 'supplier' || (daysOrOverride != null && daysOrOverride !== '' && source !== 'global')) {
        const d = Math.round(Number(days != null ? days : daysOrOverride));
        if (!Number.isFinite(d)) return 'rd:g';
        return 'rd:s:' + Math.max(0, Math.min(3650, d));
    }
    return 'rd:g';
}

/**
 * Fingerprint с учётом эффективных дней пополнения (SKU / supplier / global).
 * Список закупок джойнит кэш по CONCAT(base, '|rd:%').
 */
function buildEffectiveFormulaFingerprint(appSettings, replenishmentDaysOverride, sourceHint) {
    let source = sourceHint;
    let days = replenishmentDaysOverride;
    if (replenishmentDaysOverride && typeof replenishmentDaysOverride === 'object' && !Array.isArray(replenishmentDaysOverride)) {
        const o = replenishmentDaysOverride;
        source = o.source || o.replenishmentSource || sourceHint;
        days = o.days != null ? o.days : o.replenishmentDays;
    }
    if (!source) {
        source =
            days != null && days !== ''
    ...
    return buildFormulaFingerprint(appSettings) + '|' + formulaReplenishmentFpSuffix(days, source);
}

/**
 * SQL-фрагмент JOIN кэша формулы.
 * Плейсхолдеры: `?` = data_rev, `?` = base formula_fp (без |rd:…).
 * Любой суффикс rd:* (global / supplier / sku / legacy) — точная проверка fp в enrich.
 */
function sqlFormulaCacheJoinOnMsExport() {
    return `LEFT JOIN dg_formula_proposed_cache fc
                  ON fc.code = mse.code AND fc.data_rev = ?
                  AND fc.formula_fp LIKE CONCAT(?, '|rd:%')`;
}
...
async function upsertFormulaProposedFromProduct(db, appSettings, code, proposedFloored, windowsJson, opts) {
    ...
    const o = opts || {};
    const formulaFp = buildEffectiveFormulaFingerprint(
        appSettings,
        o.replenishmentDaysOverride != null ? o.replenishmentDaysOverride : o.replenishmentDays,
        o.replenishmentSource || o.source,
    );
