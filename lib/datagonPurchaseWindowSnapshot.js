...
const { msDemandProjectFilterClause } = require('./datagonSalesFormulaDemandFilter');
...
async function loadDirectSalesWindowsMap(db, codes, appSettings) {
    const map = new Map();
...
async function loadBundleSalesWindowsMap(db, componentCodes, appSettings) {
    const map = new Map();
 ...
 */
async function computePurchaseWindowSnapshotForItems(db, items, appSettings) {
    const out = new Map();
    ...
    const [dirWinMap, bunWinMap, absMultiMap] = await Promise.all([
        loadDirectSalesWindowsMap(db, codes, appSettings),
        loadBundleSalesWindowsMap(db, safeComponentCodes, appSettings),
