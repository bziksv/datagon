...
function createMarketplaceLogger(kind, onStep) {
            ...
            safeCtx && Object.keys(safeCtx).length
                ? ' ' +
                  Object.keys(safeCtx)
                      ...
                      .join(' ')
                : '';
        // Серверная консоль: видно все шаги обновления маркетплейса (эталон диагностики).
        // eslint-disable-next-line no-console
        console.log(`[mp:${kind}] +${tMs}ms ${msg}${ctxStr}`);
    ...
    function summary() {
        return { kind, durationMs: Date.now() - startedAt, steps: steps.slice() };
    }
    return { log, summary };
}
...
async function axiosWithMarketplaceRateLimit(config, logger, retryOpts) {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
        try {
            ...
            const retryable = st === 429 || st === 500 || st === 502 || st === 503 || st === 504;
            if (!retryable) {
...
async function exportYandexMarketRows(creds, opts) {
    ...
    logger.log('start', { maxSkus, delayMs, campaignId, businessId: businessId || null });
