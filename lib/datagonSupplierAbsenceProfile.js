...
/**
 * Профиль отсутствия поставщика «снизу вверх»:
 * dg_product_zero_stock_log (по code) + продажи → эпизоды → recommended_replenishment_days на SKU
 * → sales-weighted P90 на поставщика (подсказка для «Пополнение, дней»).
 */
...
/**
 * Рекомендация дней пополнения по SKU.
 * Эпизоды нуля задают оценку горизонта; короткий провал не даёт «рек. 1» —
 * не ниже baseline (глобаль и/или уже сохранённые дни поставщика).
 * Длинный простой поднимает рекомендацию выше baseline.
 * Без эпизодов — null.
 */
function recommendDaysForSku(maxStreak, avgEpisode, episodeCount, baselineDays) {
    if (!episodeCount || maxStreak <= 0) return null;
    const fromAbsence = Math.max(maxStreak, Math.ceil(avgEpisode || 0));
    const baseline = clampDays(
        baselineDays != null && baselineDays !== '' ? baselineDays : 30,
        1,
        3650,
    );
    return clampDays(Math.max(fromAbsence, baseline), 1, 3650);
}
