 ...
 * Габариты (длина/ширина/высота/вес) — только ручной ввод менеджеров, не подставляем.
 * Пишет в БД один раз при GET списка + строку в журнал (система / МойСклад).
 */
async function enrichMarketsRowsFromMs(db, mappedRows) {
    const list = mappedRows || [];
    let liveBudget = 25;
    for (const r of list) {
