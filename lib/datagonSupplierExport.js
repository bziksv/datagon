...
function extractUom(payload) {
    if (!payload || !payload.uom) return 'шт';
    const u = payload.uom;
    if (typeof u === 'string' && u.trim()) return u.trim();
    if (u && u.name) return String(u.name).trim() || 'шт';
    return 'шт';
}
...
function effectiveTargetStock(r) {
    const po = parseFlexibleNumber(r.proposed_min_stock);
    if (po != null) return po;
    const fc = parseFlexibleNumber(r.formula_cached_proposed);
...
async function loadSupplierExportRows(db, appSettings, opts) {
    ...
    return (rows || []).map(mapRowForExport).filter((r) => r.code);
}
