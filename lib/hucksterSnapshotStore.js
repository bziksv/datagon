...
const { HUCKSTER_MATRIX_LOST_KIND, getHucksterSyncMeta } = require('./hucksterSyncRevision');
...
async function saveHucksterSnapshot(db, payload) {
    ...
    const json = JSON.stringify({
        ...
        sync_script: payload.sync_script || getHucksterSyncMeta(),
        sheet_export: payload.sheet_export,
        sheet_export_rrc: payload.sheet_export_rrc,
        sheet_export_lost: payload.sheet_export_lost || { rows: [], total_rows: 0, matrix_kind: HUCKSTER_MATRIX_LOST_KIND },
    });
    await db.query(
