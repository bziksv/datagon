 ...
 */
/**
 * Фильтр по проектам из query (страница «Анализ поставщиков» и др.).
 * @param {object} query — req.query
 * @param {string} [alias='d']
 */
function msDemandProjectFilterFromQuery(query, alias = 'd') {
    const mode = String((query && query.project_mode) || 'all').trim().toLowerCase();
    if (mode !== 'selected') {
        return { sql: '', params: [], mode: 'all', uuids: [], fingerprint: 'all', active: false, emptySelected: false };
    }
    const uuids = parseUuidList(query && query.project_uuids);
    if (!uuids.length) {
        return {
            sql: ' AND 1=0',
            params: [],
    ...
    return {
        ...
        mode: 'selected',
        uuids,
        fingerprint: `selected:${uuids.join(',')}`,
        active: true,
        emptySelected: false,
    };
}
