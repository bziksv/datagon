...
/**
 * Сравнение линейных размеров (см). У Wildberries L/W/H в API — целые см
 * (`Math.round`), поэтому 2.5 в МС и 3 на WB считаем совпадением.
 * Вес сюда не передавать.
 */
...
function issuesDimTuplesDiffer(a, b) {
    for (let i = 0; i < 4; i += 1) {
        ...
        // Между МП: для L/W/H (0..2) учитываем целочисленность WB при сравнении пар с wb —
        // здесь оба значения уже с площадок; дробь Ozon 2.5 vs WB 3 = «разные» для across-MP.
        // Оставляем обычный EPS (между ozon 2.5 и wb 3 это расхождение витрин — ок подсветить).
